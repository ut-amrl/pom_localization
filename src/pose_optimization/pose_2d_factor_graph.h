...
namespace pose_graph {
    ...
    typedef MovableObservation<2, double, 3> MovableObservation3d;
    typedef MovableObservationFactor<2, double, 3> MovableObservationFactor3d;

    /**
     * Pose Graph containing all factors that constrain the robot's pose and the nodes representing the robot's pose.
     */
    class PoseGraph2dHeatMap2d : public PoseGraph<gp_kernel::Pose2dKernel, 3, Eigen::Quaterniond, 6, 2, double, 3> {
    public:

        PoseGraph2dHeatMap2d(gp_kernel::Pose2dKernel &kernel) : PoseGraph<gp_kernel::Pose2dKernel, 3, Eigen::Quaterniond, 6, 2, double, 3>(kernel) {

        }

        ~PoseGraph2dHeatMap2d() override {}
//
//        /**
